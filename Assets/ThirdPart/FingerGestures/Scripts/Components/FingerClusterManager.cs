...
public class FingerClusterManager : MonoBehaviour 
{
	...
	public void Update()
    {
        ...
        for( int i = 0; i < fingersRemoved.Count; ++i )
        {
            ...
            for( int clusterIndex = clusters.Count - 1; clusterIndex >= 0; --clusterIndex )
            {
                ...
                if( cluster.Fingers.Remove( finger ) )
                {
                    ...
                    {
                        //Debugger.Log( "Recycling cluster " + cluster.Id );
        ...
        for( int i = 0; i < fingersAdded.Count; ++i )
        {
            ...
            if( cluster == null )
            {
                ...
                //Debugger.Log( "Created NEW cluster " + cluster.Id + " for " + finger );                
            }
            else
            {
                //Debugger.Log( "Found existing cluster " + cluster.Id + " for " + finger );
            }
    ...
    Cluster NewCluster()
    {
        ...
        //Debugger.Log( "Created new finger cluster #" + cluster.Id );
        return cluster;
    }
