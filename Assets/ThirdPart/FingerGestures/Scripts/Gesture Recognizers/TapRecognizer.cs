...
public class TapRecognizer : DiscreteGestureRecognizer<TapGesture>
{
    ...
    {
        //Debugger.Log( "Resetting XTapRecognizer" );
        gesture.Taps = 0;
        gesture.Down = false;
    ...
    GestureRecognitionState RecognizeMultiTap( TapGesture gesture, FingerGestures.IFingerList touches )
    {
        ...
        else
        {
            ...
            if( touches.Count < RequiredFingerCount )
            {
                ...
                {
                    //Debugger.LogWarning( "MultiTap - some fingers were lifted off" );
                    return GestureRecognitionState.Failed;
                }
            }
            else // fingers were added
            {
                if( !Young( touches ) )
                {
                    //Debugger.LogWarning( "MultiTap - some fingers were added" );
                    return GestureRecognitionState.Failed;
                }
            }
        }
        ...
        if( HasTimedOut( gesture ) )
        {
            //Debugger.LogWarning( "MultiTap timed out" );
            return GestureRecognitionState.Failed;
        }
        ...
        if( gesture.Down )
        {
            ...
            if( sqrDist >= ToSqrPixels( MoveTolerance ) )
            {
                //Debugger.LogWarning( "MultiTap - moved away from original position, requesting restart" );
