...
public abstract class GestureRecognizerTS<T> : GestureRecognizer where T : Gesture, new()
{
    ...
    {
        //Debugger.Log( this.GetType().Name + " changed state from " + gesture.PreviousState + " to " + gesture.State );
    }
    ...
    protected virtual void ProcessCluster( FingerClusterManager.Cluster cluster )
    {
        ...
        if( gesture != null )
        {
            //Debugger.Log( "Gesture " + gesture + " claimed finger cluster #" + cluster.Id );
    ...
    {
        //Debugger.Log( "Beginning " + this.GetType().Name );
        ...
        // sanity check
#if UNITY_EDITOR
        //todo: if( gesture.Fingers.Count > 0 )
            //todo: Debugger.LogWarning( this.name + " begin gesture with fingers list not properly released" );
#endif
    ...
    protected virtual void UpdateGesture( T gesture, FingerGestures.IFingerList touches )
    {
        ...
        switch( gesture.State )
        {
            ...
            default:
                //todo: Debugger.LogError( this + " - Unhandled state: " + gesture.State + ". Failing gesture." );
                gesture.State = GestureRecognitionState.Failed;
                break;
        }
    }
...
public abstract class GestureRecognizer : MonoBehaviour
{
    ...
    protected virtual void OnEnable()
    {
        ...
        //todo: else
            //todo: Debugger.LogError( "Failed to register gesture recognizer " + this + " - FingerGestures instance is not available." );
    }
    ...
    protected virtual void Start()
    {
        if( !FingerGestures.Instance )
        {
            //todo: Debugger.LogWarning( "FingerGestures instance not found in current scene. Disabling recognizer: " + this );
            enabled = false;
            return;
        }
