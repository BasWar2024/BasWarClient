...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        public bool TryDelayWrapLoader(RealStatePtr L, Type type)
        {
            ...
            LuaAPI.luaL_newmetatable(L, type.FullName); //metatable
            LuaAPI.lua_pop(L, 1);
		...
		internal void collectObject(int obj_index_to_collect)
		{
			...
			if (objects.TryGetValue(obj_index_to_collect, out o))
			{
                ...
                {
                    int obj_index;
                    //lua gcweak table__gcpushluaindex
        ...
        public Array GetParams(RealStatePtr L, int index, Type type) //
        {
            Array ret = Array.CreateInstance(type, Math.Max(LuaAPI.lua_gettop(L) - index + 1, 0)); //0null
            for (int i = 0; i < ret.Length; i++)
            {
