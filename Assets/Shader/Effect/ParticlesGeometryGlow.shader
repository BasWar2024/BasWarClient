...
Shader "Game/Effect/ParticlesGeometryGlow"
{
	Properties

	{
		...
		_GlowPow("Glow Pow" , Range(0,8)) = 2
		_Glowstrength("Glow strength" , Range(0,8)) = 1
	}
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				half3 worldNor = normalize(mul(v.normal, (float3x3)unity_WorldToObject));
				float rim = max(0, dot(viewDir, worldNor));
