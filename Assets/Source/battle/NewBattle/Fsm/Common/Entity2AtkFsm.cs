...
{
    //""FSM
    public class Entity2AtkFsm : FsmState<EntityBase>
    {
        ...
        public override void OnUpdate(EntityBase owner)
        {
            ...
            if(m_IsAtk)
            {
                ...
                if (m_ReadyAtkElpaseTime >= (atkSpeed >= Fix64.One ? owner.AtkReadyTime : owner.AtkReadyTime * owner.GetFixAtkSpeed()))
                {
                    ...
                    int skillid = owner.AtkSkillId;
                    if (owner.LockedAttackEntity.IsInTheSky)
                    {
                        if (owner.AtkSkill1Id != 0)
                        {
                            skillid = owner.AtkSkill1Id;
                        }
                    }

                    var model = NewGameData._SkillModelDict[skillid];
        ...
        //    {
        //        //if (owner is SoliderBase solider)
        //        //{
        //        //    if (solider.FlashMoveDelayTime != Fix64.Zero)
        //        //    {
        ...
        private void KillLaserSkill(EntityBase owner)
        {
            if (owner.AtkLaserSkill != null)
            {
