...
function gg.table2Str(table, name, tab)
    tab = tab or ""
    name = name or ""

    local str
    if type(name) == "number" then
        str = tab .. string.format("[%s]=\n", name) .. tab .. "{"
    else
        str = tab .. string.format("[\"%s\"]=\n", name) .. tab .. "{"
    end
    ...
    for key, value in pairs(table) do
        if type(value) ~= "table" then
            local item = gg.table2Str(value, key, tab)
            str = str .. "\n" .. tab .. oneTab .. item
        else
            local item = gg.table2Str(value, key, tab .. oneTab)
            str = str .. "\n" .. item
        end
    end
    return str .. "\n" .. tab .. "},"
...
-- ""
function gg.printData(table, name)
    print(gg.table2Str(table, name))
end

-- ""
local function unPackArgs(args, count, index, ...)
    if index > count then
        return ...
    end
    return args[index], unPackArgs(args, count, index + 1, ...)
end

function gg.unPackArgs(args)
    if args then
        return unPackArgs(args, #args, 1)
    end
end

function gg.bind(func, ...)
    local args = {...}
    local count = #args
    local f = function(...)
        return func(unPackArgs(args, count, 1, ...))
    end
    return f
end

gg.spriteMap = gg.spriteMap or {}

function gg.setSpriteAsync(image, spriteName, func, param, isSetNativeSize, isHideImage)
    if not spriteName or not image or image.gameObject:Equals(nil) then
        -- if image or not image.gameObject then
        return
    end
    image.gameObject:SetActiveEx(false)

    gg.spriteMap[spriteName] = gg.spriteMap[spriteName] or {}

    local spriteInfo = gg.spriteMap[spriteName]

    if spriteInfo.sprite then

        if func then
            if not isHideImage then
                image.gameObject:SetActiveEx(true)
            end
            func(image, spriteInfo.sprite, param)
        else
            if image and not image.gameObject:Equals(nil) then
        ...
        for _, spriteData in pairs(gg.spriteMap) do
            local loadingQuene = spriteData.loadingQuene

            if loadingQuene and next(loadingQuene) then
                for i = #loadingQuene, 1, -1 do
                    if loadingQuene[i].image == image then
                        table.remove(loadingQuene, i)
                    end
                end
            end
        end
        spriteInfo.loadingQuene = spriteInfo.loadingQuene or {}
        table.insert(spriteInfo.loadingQuene, {
            func = gg.bind(gg.setSpriteAsync, image, spriteName, func, param, isSetNativeSize, isHideImage),
            image = image
        })

        if not spriteInfo.isLoading then
            spriteInfo.isLoading = true
            spriteInfo.handle = gg.resourcesManager:LoadSpriteAsync(spriteName, function(sprite)
                spriteInfo.isLoading = false
                if sprite then
                    spriteInfo.sprite = sprite
                    for index, value in ipairs(spriteInfo.loadingQuene) do
                        if not isHideImage then
                            value.image.gameObject:SetActiveEx(true)
                        end
                        if isSetNativeSize then
                            value.image:SetNativeSize()
                        end

                        value.func()
                    end
                end
                spriteInfo.loadingQuene = nil
                -- ResMgr:ReleaseAssetAsyncOperationHandle(spriteInfo.handle)
                -- spriteInfo.handle = nil
            end)
        end
    end

    -- return spriteInfo.handle
end

function gg.releaseSprite(name)
    local spriteInfo = gg.spriteMap[name]

    if spriteInfo and spriteInfo.sprite then
        ResMgr:ReleaseAssetSprite(spriteInfo.sprite)
        gg.spriteMap[name] = nil
    end
end

function gg.releaseAllSprite()
    for key, value in pairs(gg.spriteMap) do
        if value.sprite then
            ResMgr:ReleaseAssetSprite(value.sprite)
            -- ResMgr:ReleaseAssetAsyncOperationHandle(value.handle)
            gg.spriteMap[key] = nil
        end
    end
    ResMgr:ReleaseAllAtals()
end

function gg.getSpriteAtlasName(atlas, sprite)
    return string.format("%s[%s]", atlas, sprite)
end
...
local compile_cmd = setmetatable({}, {
    __index = getcmd
})
...
function reload(script)
    ...
    for k, v in pairs(package.loaded) do
        if (string.endswith(k, script)) then
...
function gg.unloadUnusedAssets()
    -- print("aaaunloadUnusedAssets")
    for key, value in pairs(gg.uiManager.closeWindows) do
        value.destroyEndTime = 0
    end

    gg.releaseAllSprite()
    ResMgr:UnloadUnusedAssetsAndGC()
