...
time.STARTTIME0 = os.time({year=1971,month=1,day=1,hour=0,min=0,sec=0})
...
function time:hms_string(secs)
    local t = self.dhms_time({hour=true,min=true,sec=true}, secs)
    local str = ""
    if t.hour ~= 0 then
        str = str..t.hour.."h"
    end
